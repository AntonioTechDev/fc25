# automation.py - Logica principale di orchestrazione dell'automazione multi-account Outlook
...
# === CONFIGURAZIONE ===
# Modifica queste variabili per personalizzare il comportamento dell'automazione
TEMPLATES_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../templates'))  # Path assoluto ai template
CSV_FILE_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../accounts.csv'))  # Path assoluto al CSV
BROWSER = 'chrome'              # Browser da usare
INCOGNITO_MODE = True           # Modalità incognito
...
CHROME_OPEN_INDEX = 0           # Step in cui aprire Chrome
MATCH_CONFIDENCE = 0.4          # Soglia matching immagini (0.0-1.0)
...
# Sequenza di automazione: ogni tupla rappresenta (template_path, testo, durata_click, [custom_delay])
AUTOMATION_SEQUENCE = [
...
def type_with_pynput(text: str, logger: logging.Logger):
...
def type_text_hybrid(text: str, logger: logging.Logger):
    if PYNPUT_AVAILABLE:
...
def find_and_interact(template_path: str, text_to_type: str = "", click_hold_duration: float = 0, logger: logging.Logger = None) -> bool:
...
def change_mac_address_and_wait(logger):
...
def run_automation(logger: logging.Logger):
    ...
    for account_index, account_data in enumerate(accounts, 1):
        ...
        try:
            ...
            if not open_browser("https://signup.live.com/", browser=BROWSER, incognito=INCOGNITO_MODE, logger=logger):
